 ...
 */
public class PagerGridLayoutManager extends PagerLinearLayoutManager {
     ...
     */
    public PagerGridLayoutManager(Context context, AttributeSet attrs, int defStyleAttr,
                                  int defStyleRes) {
     ...
     */
    public PagerGridLayoutManager(Context context, int spanCount) {
        super(context);
    ...
    public void onInitializeAccessibilityNodeInfoForItem(RecyclerView.Recycler recycler,
        ...
        if (!(lp instanceof LayoutParams)) {
            ///---- 使用反射的方式调用recyclerView.onInitializeAccessibilityNodeInfoForItem ----//
            try {
                Class<? extends Object> layoutManagerClass = getClass().getSuperclass().getSuperclass();
                Field recyclerViewField = layoutManagerClass.getDeclaredField("mRecyclerView");
                recyclerViewField.setAccessible(true);
                RecyclerView recyclerView = (RecyclerView) recyclerViewField.get(this);
                Class<? extends RecyclerView> recyclerViewClass = recyclerView.getClass();
                Method method = recyclerViewClass.getDeclaredMethod("onInitializeAccessibilityNodeInfoForItem", View.class, AccessibilityNodeInfoCompat.class);
                method.setAccessible(true);
                method.invoke(recyclerView, host, info);
            } catch (Exception e) {
                e.printStackTrace();
            }
    ...
    void layoutChunk(RecyclerView.Recycler recycler, RecyclerView.State state,
        ...
        for (int i = 0; i < count; i++) {
            ...
            if (mOrientationHelper.getDecoratedMeasurement(view) != maxSize) {
                ...
                ///---- 使用反射获取mDecorInsets ----//
                try {
                    Class<? extends LayoutParams> lpClass = lp.getClass();
                    Field decorInsetsField = lpClass.getDeclaredField("mDecorInsets");
                    decorInsetsField.setAccessible(true);
                    decorInsets = (Rect) decorInsetsField.get(lp);
                } catch (Exception e) {
                    e.printStackTrace();
                }
//                final Rect decorInsets = lp.mDecorInsets;
    ...
    private void measureChild(View view, int otherDirParentSpecMode, boolean alreadyMeasured) {
        ...
        ///---- 使用反射获取mDecorInsets ----//
        try {
            Class<?> lpClass = lp.getClass().getSuperclass();
            Field decorInsetsField = lpClass.getDeclaredField("mDecorInsets");
            decorInsetsField.setAccessible(true);
            decorInsets = (Rect) decorInsetsField.get(lp);
        } catch (Exception e) {
...
//            Class<? extends RecyclerView> recyclerViewClass = recyclerView.getClass();
            if (alreadyMeasured) {
                Method shouldReMeasureChildMethod = layoutManagerClass.getDeclaredMethod("shouldReMeasureChild", View.class, int.class, int.class, RecyclerView.LayoutParams.class);
                shouldReMeasureChildMethod.setAccessible(true);
                measure = (boolean)shouldReMeasureChildMethod.invoke(this, child, widthSpec, heightSpec, lp);
//                measure = shouldReMeasureChild(child, widthSpec, heightSpec, lp);
            } else {
                Method shouldMeasureChildMethod = layoutManagerClass.getDeclaredMethod("shouldMeasureChild", View.class, int.class, int.class, RecyclerView.LayoutParams.class);
                shouldMeasureChildMethod.setAccessible(true);
                measure = (boolean)shouldMeasureChildMethod.invoke(this, child, widthSpec, heightSpec, lp);
//                measure = shouldMeasureChild(child, widthSpec, heightSpec, lp);
     ...
     *
     * @see PagerGridLayoutManager#setSpanSizeLookup(SpanSizeLookup)
     */
