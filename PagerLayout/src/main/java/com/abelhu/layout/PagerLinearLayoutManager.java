 ...
 */
public class PagerLinearLayoutManager extends RecyclerView.LayoutManager implements
     ...
     * helps {@link PagerLinearLayoutManager} make those decisions.
     */
    ...
    /**
     * Creates a vertical PagerLinearLayoutManager
     *
     ...
     */
    public PagerLinearLayoutManager(Context context, @RecyclerView.Orientation int orientation,
     ...
     */
    public PagerLinearLayoutManager(Context context, AttributeSet attrs, int defStyleAttr,
                               int defStyleRes) {
    ...
    /**
     * Sets the orientation of the layout. {@link PagerLinearLayoutManager}
    ...
    public void setOrientation(@RecyclerView.Orientation int orientation) {
        ...
        if (orientation != mOrientation || mOrientationHelper == null) {
            // 反转Horizontal和 Vertical
            mOrientationHelper = OrientationHelper.createOrientationHelper(this, Math.abs(orientation - 1));
     ...
     *
     * <p>By default, {@link PagerLinearLayoutManager} lays out 1 extra page
     ...
     * location, where 1) the extra content helps PagerLinearLayoutManager know in advance when its
     ...
     * PagerLinearLayoutManager} and stores the result in {@code extraLayoutSpace}. {@code
     ...
     *
     * <p>By default, {@code PagerLinearLayoutManager} lays out 1 extra page of items while smooth
     ...
     * where 1) the extra content helps PagerLinearLayoutManager know in advance when its target is
    ...
    private void layoutForPredictiveAnimations(RecyclerView.Recycler recycler,
        ...
        for (int i = 0; i < scrapSize; i++) {
            ...
            ///---- 这里isRemoved不能直接调用，通过放射的方式获取 ---///
            try {
                Class<? extends RecyclerView.ViewHolder> clazz = scrap.getClass();
                Method method = clazz.getDeclaredMethod("isRemoved");
                method.setAccessible(true);
                boolean isRemoved = (boolean) method.invoke(scrap);
                if (isRemoved) {
                    continue;
                }
...
//                continue;
//            }
            ///---- end:这里isRemoved不能直接调用，通过放射的方式获取 ---///
     ...
     * <code>4</code> to this method for each inner RecyclerView's PagerLinearLayoutManager will enable
     ...
     * @see PagerLinearLayoutManager.LayoutState#mLayoutDirection
     */
     ...
     * independent from the rest of the {@link PagerLinearLayoutManager}
    ...
    int fill(RecyclerView.Recycler recycler, LayoutState layoutState,
        ...
        while ((layoutState.mInfinite || remainingSpace > 0) && layoutState.hasMore(state)) {
            ...
            ///---- 完成编译后的VERBOSE_TRACING为false，这里可以直接注释掉
...
//            }
    ...
    /// 因为父类的shouldMeasureTwice是默认的，不能Override，但是可以直接调用
    // @Override
    boolean shouldMeasureTwice() {
        ///---- hasFlexibleChildInBothOrientations 使用反射的方式调用 ----///
        try {
            Class<? extends PagerLinearLayoutManager> clazz = this.getClass();
            Method method= clazz.getDeclaredMethod("hasFlexibleChildInBothOrientations");
            method.setAccessible(true);
            boolean hasFlexibleChildInBothOrientations = (boolean)method.invoke(this);
...
//                        acceptableBoundsFlag);
        return findOneVisibleChildByReflect(fromIndex, toIndex, preferredBoundsFlag,acceptableBoundsFlag);
    }
...
//                        acceptableBoundsFlag);
    ...
    View findOneVisibleChildByReflect(int fromIndex, int toIndex, int preferredBoundsFlag, int acceptableBoundsFlag){
        ///---- mHorizontalBoundCheck 和 mVerticalBoundCheck 需要用反射去获取 ----///
        try {
            // 先拿到变量mHorizontalBoundCheck
            Class<? extends PagerLinearLayoutManager> horizontalClass = this.getClass();
            Field horizontalField = horizontalClass.getDeclaredField("mHorizontalBoundCheck");
            horizontalField.setAccessible(true);
            Object horizontal = horizontalField.get(this);
            // 再拿到horizontalView
            Class<? extends Object> horizontalCheckClass = horizontal.getClass();
            Method horizontalMethod = horizontalCheckClass.getDeclaredMethod("findOneViewWithinBoundFlags", Integer.class, Integer.class, Integer.class, Integer.class);
            View horizontalView = (View) horizontalMethod.invoke(horizontal, fromIndex, toIndex, preferredBoundsFlag, acceptableBoundsFlag);
            // 先拿到变量mHorizontalBoundCheck
            Class<? extends PagerLinearLayoutManager> verticalClass = this.getClass();
            Field verticalField = verticalClass.getDeclaredField("mVerticalBoundCheck");
            verticalField.setAccessible(true);
            Object vertical = verticalField.get(this);
            // 再拿到horizontalView
            Class<? extends Object> verticalCheckClass = vertical.getClass();
            Method verticalMethod = verticalCheckClass.getDeclaredMethod("findOneViewWithinBoundFlags", Integer.class, Integer.class, Integer.class, Integer.class);
            View verticalView = (View) verticalMethod.invoke(vertical, fromIndex, toIndex, preferredBoundsFlag, acceptableBoundsFlag);
            return (mOrientation == HORIZONTAL) ? horizontalView:verticalView;
